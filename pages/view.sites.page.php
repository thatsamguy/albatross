...
if($uri[3]=="phpmicrocache"){
  if(strtolower($uri[4]) == "disable"){
    ...
    if($result[0]){ $site_data = $site->status($auth->acc_id); }
    goto postprocessing;
  }
...
?>
  <div class="error fullwidth">
    This site's configuration has been changed but not yet applied. <a href="/sites/updateconf/<?php echo $sitename;?>">Apply this configuration now</a>.
  </div>
<?php
}
?>
<div class="block">
  ...
  <table>
  <?php
    if($thissite['phpmicrocache'] == "enabled"){ $phpmicrocache = "Disable"; }else{ $phpmicrocache = "Enable"; }
    echo "<tfoot>\n";
    echo "\t<tr>\n";
    ...
    if($thissite['phpenabled'] == "1"){ $thissite['phpenabled'] = "Enabled - Version: ".phpversion(); }else{ $thissite['phpenabled'] = "Disabled"; }
    echo "\t\t<td>".$thissite['phpenabled']."</td>\n";
    echo "\t</tr>\n";
    echo "\t<tr>\n";
    echo "\t\t<th>PHP Microcache</th>\n";
    if($thissite['phpmicrocache'] == "enabled"){ $thissite['phpmicrocache'] = "Enabled"; }else{ $thissite['phpmicrocache'] = "Disabled"; }
    echo "\t\t<td>".$thissite['phpmicrocache']."</td>\n";
    echo "\t</tr>\n";
    echo "\t\t<th>Custom Configuration</th>\n";
    if($thissite['customcode'] == "enabled"){ $thissite['customcode'] = "Enabled"; }else{ $thissite['customcode'] = "Disabled"; }

...
?>
<div class="block">
  <h3>Email servers<?php if($domain_id>0){ echo " for ".$domain;}?></h3>
  <table>
    <?php
	...
	foreach($records[1] as $key=>$record){
	  if($record['type']=="MX"){
	    if(!in_array($record['name'],$nslist)){ $nslist[] = $record['name']; }
	    echo "\t<tr>\n";
...
<div class="block">
  <h3>Update Email servers<?php if($domain_id>0){ echo " for ".$domain;}?></h3>
  <!--<form action="/dns/email/<?php echo $domain_id;?>" method="post">
  <table>
    <thead>
      <tr>
  ...
  <table>
	...
	<th>Priority</th>
      ...
      <tr>
	<td><input type="hidden" name="domain" value="<?php echo $domain;?>"><?php echo $domain;?></td>

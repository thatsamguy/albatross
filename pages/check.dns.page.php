<div class="block doublewidth">
  <h3>Check worldwide propagation of records<?php if($domain_id>0){ echo " from ".$domain;}?></h3>
  ...
  <table>
	...
	foreach($records[1] as $key=>$record){
	  if($record['type']!="SOA"){
	    ...
	    if($record['type'] != "MX"){ $record['priority'] = ""; }else{ $record['priority'] = " (".$record['priority'].")";}

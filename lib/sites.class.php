...
class site{
  ...
  public function add($acc_id,$name,$domain,$profile="default"){ // adds a new site
  ...
  public function status($acc_id){ // grabs the current status of all sites for an acc_id
    global $error;
    ...
    unset($site_name);
    $return[1]['__totalsize__'] = $totalsize; unset($totalsize);
    $return[1]['__totalbandwidth__'] = $totalbandwidth; unset($totalbandwidth);
  ...
  private function site_size($acc_id,$site_name,$raw=false){ // calculates the size of the site in human readable sizes
    global $error; global $conf;
  ...
  private function site_httpbandwidth($acc_id,$site_name){ // returns the viewed and nonviewed httpbandwidth of the site in raw
    global $error; global $conf;
  ...
  public function stats($acc_id){ // returns each site name and a url for the awstats stats page for that site
    global $error; global $conf;
  ...
  public function get_domainslist($acc_id,$site_name){ // returns the domainslist string, does not include the default domain
  ...
  public function get_redirectlist($acc_id,$site_name){ // returns the redirectlist as a nginx conf string
    ...
    foreach($result[1] as $key=>$value){
      ...
      if(strtolower($value['type'])!="redirect" AND strtolower($value['type'])!="permanent"){ $value['type'] = "redirect"; }
  ...
  public function get_customcode($acc_id,$site_name){ // returns the custom code as a nginx conf string
  ...
  public function get_php_microcache($acc_id,$site_name){ // returns true or false for enabling fcgi microcaching
  ...
  public function get_redirect_array($acc_id,$site_name){ // returns the redirectlist as an array
    $result = $this->get_attr_group($acc_id,$site_name,"redirects");
    if(!$result[0]){
      $return = $result;
      goto end;
  ...
  public function update_profile($acc_id,$site_name,$profile){ // Changes the site nginx profile
    global $error;
  ...
  public function domain_makeprimary($acc_id,$site_name,$domain){ // set domain as primary, set primary as additional domain
    global $error;
  ...
  public function sched_site_size(){ // check which sites need a size check and adds to queue
    global $error;
  ...
  public function sched_site_httpbandwidth(){ // check which sites need a size check and adds to queue
    global $error;

...
class account
{
    // TODO: migrate code to new system to avoid race condition on checking for db access
  ...
  public function __construct()
  {
    ...
    { // Creates a new account - NOTE: Only run as daemon jobpack
    global $error;
        global $conf;
        $return = '';
    ...
    { // Changes the password - NOTE: must be run as daemon jobpack
    global $error;
        $return = '';
    ...
    private function check_acc_id($account_id)
    { // Checks for a valid acc_id in the database
    global $error;
        $return = '';
        if (!$this->db->connect()) {
            $errmsg = 'unable to connect to database';

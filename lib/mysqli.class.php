...
class db{
  ...
  function __construct(){ // Alias of dbconnect()
    // start db connection
    $this->connect();
  }

  function __destruct(){ // Alias of dbclose()
    // remove db connection
    $this->close();
  }

  public function connect(){ // creates database connection
    global $error;
  ...
  public function close(){ // Closes database connection
    unset($this->sql);
  }

  public function esc($string){ //Escapes a string for use in mysql query
    global $error;

...
class email
{
    ...
    { // adds domains to database as inactive
    global $error;
    // escape domain
...
Cyprix Webmaster
  ";
    ...
    public function update_email_name($email, $name)
    { // updates account name for the email address
    return $this->update_email($email, 'name', $name);
    }

    public function add_alias($alias, $destination)
    { // adds aliases to database as active
    global $error;
    // escape alias
    ...
    public function get_domains($active = '-1')
    { // returns array of domains in database (all or matching active)
    global $error;
    ...
    { // returns array of data for a single email account
    global $error;
    ...
    { // returns data on a single alias
    global $error;
    ...
    public function get_alias_by_destination($destination)
    { // returns an array of aliases for a given email address
    global $error;
    ...
    public function email_size($acc_id, $email, $raw = false)
    { // calculates the size of the email address in human or raw sizes
    global $error;
        global $conf;
    ...
    public function sched_email_size()
    { // check which emails need a size check and adds to queue
    global $error;
                ...
                foreach ($temp as $num => $value) {
                    if ($value == 'dovecot:' and stripos($line, 'bytes') !== false) {
                        ...
                        if ($acc_check[0]) {
                            ...
                            if (strtolower(substr($line, 0, 3)) != strtolower(date('M')) and date('M') == 'Jan') {
                                $year = date('Y') - 1;
                            } else {
                                $year = date('Y');
                            }
